...
//    }
  ...
  char container_primary_type [PROPERTY_VALUE_MAX];
  hwc_get_string_property("ro.container.primary_type", "",container_primary_type);
  ALOGD("ro.container.primary_type is %s",container_primary_type);
  if(strlen(container_primary_type)==0){
    ALOGD("ro.container.primary_type is not set, use normal mode");
    ...
    std::set<std::string> containerConnectors;
    std::string cc_str(container_primary_type);
    size_t comm_pos;
    do{
        comm_pos = cc_str.find(",");
        if(comm_pos != std::string::npos){
            containerConnectors.insert(cc_str.substr(0,comm_pos));
            cc_str = cc_str.substr(comm_pos+1);
        }
    }while(comm_pos!=std::string::npos);
    containerConnectors.insert(cc_str);

    std::set<int> crtc_indexs;
    for (int i = 0; !ret && i < res->count_connectors; ++i) {
      ...
      char connector_name [PROPERTY_VALUE_MAX];
      sprintf(connector_name, "%s-%u",
			 connector_type_str(c->connector_type), c->connector_type_id);

      if(containerConnectors.count(std::string(connector_name))){
        for (int j = 0; !ret && j < res->count_encoders; ++j) {
          drmModeEncoderPtr e = drmModeGetEncoder(fd(), res->encoders[j]);
          if (!e) {
            ALOGE("Failed to get encoder %d", res->encoders[j]);
            ret = -ENODEV;
            break;
          }
          for (int k = 0; k < c->count_encoders; ++k)
            if(c->encoders[k]==e->encoder_id){
              int c_no = 0;
              if(e->possible_crtcs==0)
                continue;
              else{
                while(!((1<<c_no)&e->possible_crtcs)){
                  c_no++;
                }
                ALOGD("FIND CRTC%d",c_no);
                crtc_indexs.insert(c_no);
              }
            }
    ...
    if(crtc_indexs.size()==0){
      ALOGE("FAILED TO FIND CRTC");
      return std::make_tuple(ret, 0);
    }
    for(auto found_crtc:crtc_indexs){
      drmModeCrtcPtr c = drmModeGetCrtc(fd(), res->crtcs[found_crtc]);
        if (!c) {
          ALOGE("Failed to get crtc %d", res->crtcs[found_crtc]);
          ret = -ENODEV;
          return std::make_tuple(ret, 0);
        }

        std::unique_ptr<DrmCrtc> crtc(new DrmCrtc(this, c, found_crtc));
        drmModeFreeCrtc(c);

        ret = crtc->Init();
        if (ret) {
          ALOGE("Failed to initialize crtc %d", res->crtcs[found_crtc]);
          return std::make_tuple(ret, 0);
        }
        soc_id_ = crtc->get_soc_id();
        crtcs_.emplace_back(std::move(crtc));
