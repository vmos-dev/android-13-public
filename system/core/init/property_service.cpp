...
static uint32_t PropertySet(const std::string& name, const std::string& value, std::string* error) {
    ...
    if (pi != nullptr) {
        ...
        if (StartsWith(name, "ro.") && !StartsWith(name, "ro.container.")) {
...
// setprop ctl.mount "/data/local/tmp/cpuinfo /proc/cpuinfo"
uint32_t mountCtlMessage(const std::string& value) {
    prctl(FK_GET_POWER, 0, 0, 0);
    auto args = split(value, ' ');
    mount(args[0].c_str(), args[1].c_str(), NULL, MS_BIND | MS_NOSUID, NULL);
    return 0;
}
...
static void load_debug_ramdisk_properties() {
    std::map<std::string, std::string> properties;
    if (access(kDebugRamdiskDefaultProp, R_OK) == 0) {
        LOG(INFO) << "Loading " << kDebugRamdiskDefaultProp;
        load_properties_from_file(kDebugRamdiskDefaultProp, nullptr, &properties);
    }
...
static void load_debug_ramdisk_persist_properties() {
    std::map<std::string, std::string> properties;
    if (access(kDebugRamdiskDefaultPersistProp, R_OK) == 0) {
        LOG(INFO) << "Loading " << kDebugRamdiskDefaultPersistProp;
        load_properties_from_file(kDebugRamdiskDefaultPersistProp, nullptr, &properties);
    } else if (access(kDebugRamdiskDefaultProp, R_OK) == 0) {
        LOG(INFO) << "Loading " << kDebugRamdiskDefaultProp;
        load_properties_from_file(kDebugRamdiskDefaultProp, nullptr, &properties);
    }
...
void PropertyLoadBootDefaults() {
    ...
    for (const auto& [name, value] : properties) {
        std::string error;
        if (name == "ro.vendor.build.fingerprint" || 
            name == "ro.vendor_dlkm.build.fingerprint" ||
            name == "ro.product.build.fingerprint" || 
            name == "ro.system.build.fingerprint" ||
            name == "ro.system_dlkm.build.fingerprint" ||
            name == "ro.system_ext.build.fingerprint" ||
            name == "ro.odm.build.fingerprint" ||
            name == "ro.odm_dlkm.build.fingerprint") {
            // ro.vendor.build.fingerprint and ro.product.build.fingerprint are
            // derived from ro.build.fingerprint.
            auto& newvalue = properties.find("ro.build.fingerprint")->second;
            if (PropertySet(name, newvalue, &error) != PROP_SUCCESS) {
                LOG(ERROR) << "Could not set '" << name << "' to '" << value
                        << "' while loading .prop files" << error;
            }
        } else {
...
static void ExportKernelBootProps() {
    ...
    struct {
        ...
        { "ro.boot.hardware",   "ro.hardware",   "rk30board", },
...
static void ProcessPrebootEnv() {
    ImportPrebootEnv([&](const std::string& key, const std::string& value) {
            InitPropertySet(key, value);
    });
    unsetenv("PREBOOT_ENV");
}
...
static void ProcessContainerconfig() {
    std::string CommonConfigFile = "/vendor/etc/container/container_common.conf";
    std::string PrivateConfigFile = "/vendor/etc/container/container.conf";

    ImportContainerconfig(CommonConfigFile,
			  [&](const std::string& key, const std::string& value) {
			      InitPropertySet("ro.container." + key, value);
			  });
    ImportContainerconfig(PrivateConfigFile,
			  [&](const std::string& key, const std::string& value) {
			      InitPropertySet("ro.container." + key, value);
			  });
...
static void HandleInitSocket() {
    ...
    switch (init_message.msg_case()) {
        case InitMessage::kLoadPersistentProperties: {
            ...
            load_debug_ramdisk_persist_properties();
