...
void ImportPrebootEnv(const std::function<void(const std::string&, const std::string&)>& fn) {
    std::string cmdline = getenv("PREBOOT_ENV");
...
void ImportContainerconfig(const std::string& filename, const std::function<void(const std::string&, const std::string&)>& fn) {
    std::string containerconfig;

	if(isFileExists(filename)) {
		android::base::ReadFileToString(filename, &containerconfig);
	} else {
		LOG(INFO) << "container config file not found:" << filename;
		return;
	}
    LOG(INFO) << "read form:" << filename << ":" << containerconfig;
    for (auto& entry : android::base::Split(containerconfig, "\n")) {
		//erase comment line
		size_t hashPos = entry.find('#'); // Find the position of '#'
        if (hashPos != std::string::npos) {
            entry.erase(hashPos); // Erase all characters starting from '#'
        }
...
std::string GetContainerId() {
    std::string containerconfig;
    std::string targetKey="container_id";
    std::string ConfigFile = "/vendor/etc/container/container.conf";
    std::unordered_map<std::string, std::string> configMap;

    if(isFileExists(ConfigFile)) {
        android::base::ReadFileToString(ConfigFile, &containerconfig);
    } else {
        LOG(INFO) << "container config file not found:" << ConfigFile;
        return {};
    }
    ...
    for (auto& entry : android::base::Split(containerconfig, "\n")) {
        ...
        if (pieces.size() == 2) {
            ...
            std::string value = android::base::StringReplace(pieces[1], "\", \"", ",", true);
            value.erase(std::remove(value.begin(), value.end(), '"'), value.end());
            configMap[android::base::Trim(pieces[0])] = android::base::Trim(value);
        }
    }

    auto it = configMap.find(targetKey);
    if (it != configMap.end()) {
        //LOG(INFO) << "Found '" << targetKey << "' in configuration. Value: " << it->second;
        return it->second;
    } else {
        LOG(INFO) << "Key '" << targetKey << "' not found in configuration.";
        return {};
    }
}
...
void InitKernelLogging(char** argv) {
    ...
    // add container_id tag for init log
    std::string container_id = GetContainerId();
    if (!container_id.empty()) {
        char* newArgv0 = new char[container_id.length() + strlen(argv[0]) + strlen(": container_") + 1];
        sprintf(newArgv0, "%s: container_%s", argv[0], container_id.c_str());
        argv[0] = newArgv0;
    }
