...
#if (RTL8822E_SUPPORT)
#if (PHYDM_FW_API_ENABLE_8822E)
/* ======================================================================== */
/* These following functions can be used for PHY DM only*/
#ifdef CONFIG_TXAGC_DEBUG_8822E
__odm_func__
boolean phydm_set_pw_by_rate_8822e(struct dm_struct *dm, s8 *pw_idx,
				   u8 rate_idx)
{
	u32 pw_all = 0;
	u8 j = 0;
	...
	for (j = 0; j < 4; j++)
		config_phydm_read_txagc_diff_8822e(dm, rate_idx + j);

	return true;
}

__odm_func__
void phydm_txagc_tab_buff_init_8822e(struct dm_struct *dm)
{
	u8 i;
...
#if 0
__odm_func__
void phydm_restore_afedig_8822e(struct dm_struct *dm)
{
	odm_set_bb_reg(dm, R_0x3804, MASKDWORD, 0x45800000);
	odm_set_bb_reg(dm, R_0x3830, MASKDWORD, 0x45800000);
	odm_set_bb_reg(dm, R_0x3904, MASKDWORD, 0x45800000);
	odm_set_bb_reg(dm, R_0x3930, MASKDWORD, 0x45800000);
}
#endif

__odm_func__
void phydm_rstb_3wire_8822e(struct dm_struct *dm, boolean enable)
{
	if (enable) {
...
__odm_func__
void phydm_sdm_reset_8822e(struct dm_struct *dm)
{
	/*reset HSSI*/
	//phydm_rstb_3wire_8822e(dm, false);
	/*write RF-0x18*/
...
#if 0
void phydm_dac_fifo_reset_8822e(struct dm_struct *dm)
{
	/*dac_fifo_reset*/
...
__odm_func__
void phydm_bb_reset_8822e(struct dm_struct *dm)
{
	//if (*dm->mp_mode)
		//return;
...
__odm_func__
void phydm_bb_reset_no_3wires_8822e(struct dm_struct *dm)
{
	/* Disable PI hardware */
	//odm_set_bb_reg(dm, R_0x1c90, BIT(8), 0x0);
				      ...
				      u8 ver_bb, u8 ver_rf)
{
	...
	if (ver_bb >= 18 && ver_rf >= 12)
		valid = true;
	else if (ver_bb < 18 && ver_rf < 12)
		valid = true;
	else
		valid = false;
...
__odm_func__
void phydm_igi_toggle_8822e(struct dm_struct *dm)
{
/*
...
__odm_func__
u32 phydm_check_bit_mask_8822e(u32 bit_mask, u32 data_original, u32 data)
{
	u8 bit_shift = 0;
...
__odm_func__
boolean
config_phydm_direct_write_rf_reg_8822e(struct dm_struct *dm, enum rf_path path,
...
__odm_func__
boolean
config_phydm_write_rf_reg_8822e(struct dm_struct *dm, enum rf_path path,
				u32 reg_addr, u32 bit_mask, u32 data)
{
	...
	if (!(reg_addr == RF_0x0)) {
		result = config_phydm_direct_write_rf_reg_8822e(dm, path,
								reg_addr,
								bit_mask, data);
		return result;
	}
	...
	if (bit_mask != RFREG_MASK) {
		data_original = config_phydm_read_rf_reg_8822e(dm, path,
							       reg_addr,
							       RFREG_MASK);
		...
		/* @check bit mask */
		data = phydm_check_bit_mask_8822e(bit_mask, data_original,
						  data);
	}
...
__odm_func__
boolean
phydm_write_txagc_1byte_8822e(struct dm_struct *dm, u32 pw_idx, u8 hw_rate)
{
#if (PHYDM_FW_API_FUNC_ENABLE_8822E)
	...
	for (i = 0; i < 4; i++) {
		if (i != rate_offset) {
			ret = config_phydm_read_txagc_diff_8822e(dm,
								 rate_idx + i);
			txagc_idx |= ret << (i << 3);
		} else {
...
__odm_func__
boolean
config_phydm_write_txagc_ref_8822e(struct dm_struct *dm, u8 power_index,
				   enum rf_path path,
...
__odm_func__
boolean
config_phydm_write_txagc_diff_8822e(struct dm_struct *dm, s8 power_index1,
...
void config_phydm_set_txagc_to_hw_8822e(struct dm_struct *dm)
{
	...
	config_phydm_write_txagc_ref_8822e(dm, ref_pow_cck[0], RF_PATH_A,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8822e(dm, ref_pow_cck[1], RF_PATH_B,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8822e(dm, ref_pow_ofdm[0], RF_PATH_A,
					   PDM_OFDM);
	config_phydm_write_txagc_ref_8822e(dm, ref_pow_ofdm[1], RF_PATH_B,
					   PDM_OFDM);
	...
	for (i = ODM_RATE1M; i <= ODM_RATEMCS15; i++) {
		...
		#endif
		if  (i % 4 == 3) {
			config_phydm_write_txagc_diff_8822e(dm,
							    diff_tab_min[j - 3],
							    diff_tab_min[j - 2],
							    diff_tab_min[j - 1],
							    diff_tab_min[j],
...
__odm_func__
boolean config_phydm_write_txagc_8822e(struct dm_struct *dm, u32 pw_idx,
				    ...
				    struct txagc_table_8822e *tab)
{
	...
	/* === [Reference base] =============================================*/
	/*Set OFDM/CCK Ref. power index*/
	config_phydm_write_txagc_ref_8822e(dm, tab->ref_pow_cck[0], RF_PATH_A,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8822e(dm, tab->ref_pow_cck[1], RF_PATH_B,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8822e(dm, tab->ref_pow_ofdm[0], RF_PATH_A,
					   PDM_OFDM);
	config_phydm_write_txagc_ref_8822e(dm, tab->ref_pow_ofdm[1], RF_PATH_B,
					   PDM_OFDM);
...
__odm_func__
void
phydm_get_tx_path_en_setting_8822e(struct dm_struct *dm,
...
__odm_func__
void
phydm_get_rx_path_en_setting_8822e(struct dm_struct *dm,
...
__odm_func__
void
phydm_config_cck_tx_path_8822e(struct dm_struct *dm, enum bb_path tx_path)
{
	...
	phydm_bb_reset_8822e(dm);
}

__odm_func__
boolean
phydm_config_cck_rx_path_8822e(struct dm_struct *dm, enum bb_path rx_path)
{
...
__odm_func__
void
phydm_config_ofdm_tx_path_8822e(struct dm_struct *dm, enum bb_path tx_path_2ss,
...
__odm_func__
void
phydm_config_ofdm_rx_path_8822e(struct dm_struct *dm, enum bb_path rx_path)
{
	u32 ofdm_rx = 0x0;
				...
				enum bb_path tx_path_sel_cck)
{
	...
	phydm_config_cck_tx_path_8822e(dm, tx_path_sel_cck);

	/* @OFDM TX antenna mapping*/
	phydm_config_ofdm_tx_path_8822e(dm, tx_path_2ss, tx_path_sel_1ss);
...
__odm_func__
void phydm_config_rx_path_8822e(struct dm_struct *dm, enum bb_path rx_path)
{
	/* @CCK RX antenna mapping */
	phydm_config_cck_rx_path_8822e(dm, rx_path);

	/* @OFDM RX antenna mapping*/
	phydm_config_ofdm_rx_path_8822e(dm, rx_path);
...
__odm_func__
void
phydm_set_rf_mode_table_8822e(struct dm_struct *dm,
...
__odm_func__
void
phydm_rfe_8822e(struct dm_struct *dm, enum bb_path path)
{
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "[8822E] Update RFE PINs: T/RX_path:{0x%x, 0x%x}, rfe_type:%d\n",
...
__odm_func__
boolean
config_phydm_trx_mode_8822e(struct dm_struct *dm, enum bb_path tx_path_en,
			    enum bb_path rx_path, enum bb_path tx_path_sel_1ss)
{
	...
	/* @==== [RF Mode Table] ========================================*/
	phydm_set_rf_mode_table_8822e(dm, tx_path_mode_table, rx_path);

	/* @==== [RX Path] ==============================================*/
	phydm_config_rx_path_8822e(dm, rx_path);
...
#endif
	phydm_config_tx_path_8822e(dm, tx_path_2ss, tx_path_sel_1ss,
				   tx_path_sel_1ss);
	...
	phydm_igi_toggle_8822e(dm);

	return true;
}

void phydm_cck_rxiq_8822e(struct dm_struct *dm, u8 set_type)
{
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "%s ======>\n", __func__);
...
__odm_func__
boolean
config_phydm_switch_band_8822e(struct dm_struct *dm, u8 central_ch)
{
	return true;
}
...
__odm_func__
void
phydm_cck_tx_shaping_filter_8822e(struct dm_struct *dm, u8 central_ch)
{
	if (central_ch == 14) {
...
__odm_func__
void
phydm_cck_agc_tab_sel_8822e(struct dm_struct *dm, u8 table)
{
...
__odm_func__
void
phydm_ofdm_agc_tab_sel_8822e(struct dm_struct *dm, u8 table)
{
	...
	if (!dm->l_bnd_detect[table])
		lower_bound = L_BND_DEFAULT_8822E;
...
__odm_func__
void
phydm_sco_trk_fc_setting_8822e(struct dm_struct *dm, u8 central_ch)
{
...
__odm_func__
void
phydm_tx_dfir_setting_8822e(struct dm_struct *dm, u8 central_ch)
{
	if (central_ch <= 14) {
		if (central_ch == 13)
...
__odm_func__
void phydm_set_manual_nbi_8822e(struct dm_struct *dm, boolean en_manual_nbi,
				int tone_idx)
{
	if (en_manual_nbi) {
		/*set tone_idx*/
...
__odm_func__
void phydm_set_nbi_wa_para_8822e(struct dm_struct *dm, boolean en_nbi,
...
__odm_func__
void phydm_csi_mask_enable_8822e(struct dm_struct *dm, boolean enable)
{
...
__odm_func__
void phydm_set_csi_mask_8822e(struct dm_struct *dm, u32 tone_idx)
{
	...
	if (tone_idx & 0x1)
		odm_set_bb_reg(dm, R_0x1d94, 0xf0, 0xa);
	else
		odm_set_bb_reg(dm, R_0x1d94, 0xf, 0xa);
...
__odm_func__
void phydm_clean_all_csi_mask_8822e(struct dm_struct *dm)
{
	u8 i = 0;
...
__odm_func__
void phydm_spur_eliminate_8822e(struct dm_struct *dm, u8 central_ch)
{
	phydm_set_auto_nbi_8822e(dm, false);
	phydm_csi_mask_enable_8822e(dm, true);

	if (central_ch == 153 && (*dm->band_width == CHANNEL_WIDTH_20)) {
		phydm_set_manual_nbi_8822e(dm, true, 112); /*5760 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 112);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x60e0); /*packet detection modified in CH153*/
	} else if (central_ch == 161 && (*dm->band_width == CHANNEL_WIDTH_20)) {
		phydm_set_manual_nbi_8822e(dm, true, 112);
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 112);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x60e0); /*packet detection modified in CH161*/
	} else if (central_ch == 169 && (*dm->band_width == CHANNEL_WIDTH_20)) {
		phydm_set_manual_nbi_8822e(dm, true, 112);
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 112);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x60e0); /*packet detection modified in CH169*/
	} else if (central_ch == 151 && (*dm->band_width == CHANNEL_WIDTH_40)) {
		phydm_set_manual_nbi_8822e(dm, true, 16); /*5760 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 16);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00e0); /*packet detection modified in CH151*/
	} else if (central_ch == 159 && (*dm->band_width == CHANNEL_WIDTH_40)) {
		phydm_set_manual_nbi_8822e(dm, true, 16);
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 16);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00e0); /*packet detection modified in CH159*/
	} else if (central_ch == 167 && (*dm->band_width == CHANNEL_WIDTH_40)) {
		phydm_set_manual_nbi_8822e(dm, true, 16);
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 16);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00e0); /*packet detection modified in CH167*/
	} else if (central_ch == 155 && (*dm->band_width == CHANNEL_WIDTH_80)) {
		phydm_set_manual_nbi_8822e(dm, true, 208); /*5760 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 208);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0xc0e0); /*packet detection modified in CH155*/
	} else if (central_ch == 171 && (*dm->band_width == CHANNEL_WIDTH_80)) {
		phydm_set_manual_nbi_8822e(dm, true, 208);
		phydm_set_nbi_wa_para_8822e(dm, true, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 208);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0xc0e0); /*packet detection modified in CH171*/
	} else if (central_ch == 54 && (*dm->band_width == CHANNEL_WIDTH_40)) {
		phydm_set_manual_nbi_8822e(dm, false, 0); /*5280 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, false, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 32);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00ff); /*packet detection set to default value*/
	} else if (central_ch == 58 && (*dm->band_width == CHANNEL_WIDTH_80)) {
		phydm_set_manual_nbi_8822e(dm, false, 0); /*5280 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, false, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 224);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00ff); /*packet detection set to default value*/
	} else if (central_ch == 118 && (*dm->band_width == CHANNEL_WIDTH_40)) {
		phydm_set_manual_nbi_8822e(dm, false, 0); /*5600 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, false, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 32);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00ff); /*packet detection set to default value*/
	} else if (central_ch == 122 && (*dm->band_width == CHANNEL_WIDTH_80)) {
		phydm_set_manual_nbi_8822e(dm, false, 0); /*5600 MHz*/
		phydm_set_nbi_wa_para_8822e(dm, false, *dm->band_width);
		phydm_set_csi_mask_8822e(dm, 224);
		odm_set_bb_reg(dm, R_0xc24, MASKLWORD, 0x00ff); /*packet detection set to default value*/
	} else {
		phydm_set_manual_nbi_8822e(dm, false, 0);
		phydm_set_nbi_wa_para_8822e(dm, false, *dm->band_width);
...
__odm_func__
void phydm_set_dis_dpd_by_rate_8822e(struct dm_struct *dm, u16 bitmask)
{
	/* bit(0) : ofdm 6m*/
	/* bit(1) : ofdm 9m*/
	/* bit(2) : ht mcs0*/
...
config_phydm_switch_channel_8822e(struct dm_struct *dm, u8 central_ch)
{
	...
	phydm_sco_trk_fc_setting_8822e(dm, central_ch);
	/* 3. TX DFIR*/
	phydm_tx_dfir_setting_8822e(dm, central_ch);
	/* 4. Other BB Settings*/
	if (is_2g_ch) {
		phydm_cck_tx_shaping_filter_8822e(dm, central_ch);
		/* Enable CCK Rx IQ */
		phydm_cck_rxiq_8822e(dm, PHYDM_SET);
	...
	if (iot_table->patch_id_011f0500) {
		if (central_ch != 1 && dm->en_dis_dpd)
			phydm_set_dis_dpd_by_rate_8822e(dm, 0x3ff);
		else
			phydm_set_dis_dpd_by_rate_8822e(dm, 0x0);
	}
#endif
	/*====================================================================*/
	phydm_spur_eliminate_8822e(dm, central_ch);
...
__odm_func__
boolean
config_phydm_switch_bandwidth_8822e(struct dm_struct *dm, u8 pri_ch,
				    enum channel_width bw)
{
	u32 rf_reg18 = 0;
	u32 rf_reg1a = 0;
	...
	case CHANNEL_WIDTH_20:
		...
		/*RF TXBB&RXBB setting*/
		rf_reg1a |= (BIT(11) | BIT(10));
	...
	odm_set_rf_reg(dm, RF_PATH_B, RF_0x18, RFREG_MASK, rf_reg18);
	/*write RF-0x1a*/
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x1a, RFREG_MASK, rf_reg1a);
	...
	/*fix dac fifo error after TXCK setting*/
#if (defined(CONFIG_RUN_IN_DRV))
...
__odm_func__
boolean
config_phydm_switch_channel_bw_8822e(struct dm_struct *dm, u8 central_ch,
				     u8 primary_ch_idx,
				     enum channel_width bandwidth)
{
	/* @Switch channel */
	if (!config_phydm_switch_channel_8822e(dm, central_ch))
		return false;

	/* @Switch bandwidth */
	if (!config_phydm_switch_bandwidth_8822e(dm, primary_ch_idx, bandwidth))
		return false;
...
{
	if (central_ch == 153 || central_ch == 151 || central_ch == 155) {
		odm_set_bb_reg(dm, R_0x1cdc, BIT(31), 0x1);
	} else {
		odm_set_bb_reg(dm, R_0x1cdc, BIT(31), 0x0);
	}
}
...
void phydm_cck_pd_init_8822e(struct dm_struct *dm)
{
#if (defined(CONFIG_RUN_IN_DRV))
	struct phydm_iot_center	*iot_table = &dm->iot_table;
...
__odm_func__
boolean
config_phydm_parameter_init_8822e(struct dm_struct *dm,
				  enum odm_parameter_init type)
{
	...
	phydm_cck_gi_bound_8822e(dm);
	phydm_cck_pd_init_8822e(dm);

	if (*dm->mp_mode)
		phydm_ch_smooth_setting_8822e(dm, true);
	...
	phydm_bb_reset_8822e(dm);
	#ifdef CONFIG_TXAGC_DEBUG_8822E
	/*phydm_txagc_tab_buff_init_8822e(dm);*/
	#endif
...
__odm_func__
boolean
phydm_chk_bb_state_idle_8822e(struct dm_struct *dm)
{
	u32 dbgport = 0;
...
__odm_func_aon__
boolean
phydm_8822e_lps(struct dm_struct *dm, boolean enable_lps)
{
...
/* ======================================================================== */
#endif /* PHYDM_FW_API_ENABLE_8822E */
