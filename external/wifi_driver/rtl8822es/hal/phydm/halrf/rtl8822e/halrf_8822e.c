	...
	struct dm_struct *dm)
{
	...
	u8 channel = *dm->channel;
	u32 rf0_b, i, j, psd_result[16] = {0};
	u32 psd_result_zero_num = 0;
	u32 min_psd_idx, temp;
	u32 psd_start_point = 4;
	u32 reg_0x824, reg_0x1b1c;
	...
	if (channel == 151 || channel == 153 || channel == 155) {
		...
		/*RF*/
		odm_set_rf_reg(dm, RF_PATH_B, RF_0x0, RFREGOFFSETMASK, 0x31c20);
		...
		for (i = psd_start_point; i < 16; i++) {
			...
			psd_result[i] = odm_get_bb_reg(dm, R_0x1bfc, MASKDWORD);
			rxspurk->rxspurk_psd_result[i] = psd_result[i];

			RF_DBG(dm, DBG_RF_RXSPURK, "[SpurK] psd_result[%02d]=0x%08x\n",
				i, psd_result[i]);
		}
		...
		if (psd_result_zero_num == 16) {
			min_psd_idx = 8;
		} else {
			temp = psd_result[psd_start_point];
			min_psd_idx = 4;
			for (i = psd_start_point + 1; i < 16; i++) {
...
void halrf_rxspurk_info_8822e(void *dm_void, u32 *_used, char *output, u32 *_out_len)
{
	...
	PDM_SNPF(*_out_len, *_used, output + *_used, *_out_len - *_used, "%-25s = 0x%02x\n",
		" PX Spur K final idx", rxspurk->final_psd_idx);

	for (i = 0; i < 16; i++)
		PDM_SNPF(*_out_len, *_used, output + *_used, *_out_len - *_used, "%02d %-22s = 0x%08x\n",
			i, " PSD Result", rxspurk->rxspurk_psd_result[i]);
