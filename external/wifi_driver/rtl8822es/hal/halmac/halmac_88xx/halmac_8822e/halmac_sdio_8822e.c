...
#include "halmac_sdio_8822e.h"
#include "halmac_pwr_seq_8822e.h"
#include "../halmac_init_88xx.h"
#include "../halmac_common_88xx.h"
#include "../halmac_sdio_88xx.h"

#if (HALMAC_8822E_SUPPORT && HALMAC_SDIO_SUPPORT)

#define WLAN_ACQ_NUM_MAX	8

static enum halmac_ret_status
chk_oqt_8822e(struct halmac_adapter *adapter, u32 tx_agg_num, u8 *buf,
	      u8 macid_cnt);

static enum halmac_ret_status
update_oqt_free_space_8822e(struct halmac_adapter *adapter);

static enum halmac_ret_status
update_sdio_free_page_8822e(struct halmac_adapter *adapter);

static enum halmac_ret_status
update_ac_empty_8822e(struct halmac_adapter *adapter, u8 value);

static enum halmac_ret_status
chk_qsel_8822e(struct halmac_adapter *adapter, u8 qsel_first, u8 *pkt,
	       u8 *macid_cnt);

static enum halmac_ret_status
chk_dma_mapping_8822e(struct halmac_adapter *adapter, u16 **cur_fs,
		      u8 qsel_first);

static enum halmac_ret_status
chk_rqd_page_num_8822e(struct halmac_adapter *adapter, u8 *buf, u32 *rqd_pg_num,
		       u16 **cur_fs, u8 *macid_cnt, u32 tx_agg_num);

/**
 * init_sdio_cfg_8822e() - init SDIO
...
init_sdio_cfg_8822e(struct halmac_adapter *adapter)
{
	u32 value32;
...
/**
 * mac_pwr_switch_sdio_8822e() - switch mac power
 * @adapter : the adapter of halmac
 * @pwr : power state
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
mac_pwr_switch_sdio_8822e(struct halmac_adapter *adapter,
			  enum halmac_mac_power pwr)
{
	...
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
	PLTFM_MSG_TRACE("[TRACE]8822E pwr seq ver = %s\n",
			HALMAC_8822E_PWR_SEQ_VER);
	...
	if (pwr == HALMAC_MAC_POWER_OFF) {
		...
		if (pwr_seq_parser_88xx(adapter, card_dis_flow_8822e) !=
		    HALMAC_RET_SUCCESS) {
		...
		if (pwr_seq_parser_88xx(adapter, card_en_flow_8822e) !=
		    HALMAC_RET_SUCCESS) {
 ...
 */
enum halmac_ret_status
tx_allowed_sdio_8822e(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u16 *cur_fs = NULL;
	u32 cnt;
	u32 tx_agg_num;
	...
	status = chk_rqd_page_num_8822e(adapter, buf, &rqd_pg_num, &cur_fs,
	...
	do {
		if ((u32)(*cur_fs + fs_info->pubq_pg_num) > rqd_pg_num) {
			status = chk_oqt_8822e(adapter, tx_agg_num, buf,
					       macid_cnt);
			if (status != HALMAC_RET_SUCCESS) {
		...
		update_sdio_free_page_8822e(adapter);
...
/**
 * cfg_tx_fmt_sdio_8822e() - config sdio tx address format
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_tx_fmt_sdio_8822e(struct halmac_adapter *adapter,
 ...
 */
u8
reg_r8_sdio_8822e(struct halmac_adapter *adapter, u32 offset)
{
	u8 value8;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
 ...
 */
enum halmac_ret_status
reg_w8_sdio_8822e(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_sdio_cmd53_4byte_mode cmd53_4byte =
						adapter->sdio_cmd53_4byte;
 ...
 */
u16
reg_r16_sdio_8822e(struct halmac_adapter *adapter, u32 offset)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	union {
		__le16 word;
 ...
 */
enum halmac_ret_status
reg_w16_sdio_8822e(struct halmac_adapter *adapter, u32 offset, u16 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
 ...
 */
u32
reg_r32_sdio_8822e(struct halmac_adapter *adapter, u32 offset)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	union {
		__le32 dword;
 ...
 */
enum halmac_ret_status
reg_w32_sdio_8822e(struct halmac_adapter *adapter, u32 offset, u32 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	      ...
	      u8 macid_cnt)
{
	...
	case HALMAC_QSEL_BK_V2:
		...
		do {
			...
			update_oqt_free_space_8822e(adapter);
	...
	case HALMAC_QSEL_HIGH:
		if (tx_agg_num > OQT_ENTRY_NOAC_8822E)
...
update_oqt_free_space_8822e(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8822e(adapter, data[4]);
...
update_sdio_free_page_8822e(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8822e(adapter, data[12]);
...
update_ac_empty_8822e(struct halmac_adapter *adapter, u8 value)
{
	...
	if (free_space->ac_oqt_num == OQT_ENTRY_AC_8822E) {
		while (value > 0) {
			value = value & (value - 1);
			free_space->ac_empty++;
		};
		...
		PLTFM_MSG_TRACE("[TRACE]free_space->ac_oqt_num %d != %d\n",
				free_space->ac_oqt_num, OQT_ENTRY_AC_8822E);
	}
 ...
 */
enum halmac_ret_status
phy_cfg_sdio_8822e(struct halmac_adapter *adapter,
 ...
 */
enum halmac_ret_status
pcie_switch_sdio_8822e(struct halmac_adapter *adapter,
...
/**
 * intf_tun_sdio_8822e() - sdio interface fine tuning
 * @adapter : the adapter of halmac
 * Author : Ivan
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
intf_tun_sdio_8822e(struct halmac_adapter *adapter)
{
	return HALMAC_RET_SUCCESS;
}
 ...
 */
enum halmac_ret_status
get_sdio_tx_addr_8822e(struct halmac_adapter *adapter, u8 *buf, u32 size,
		       u32 *cmd53_addr)
{
	       ...
	       u8 *macid_cnt)
{
	...
	if (qsel_first == qsel_now) {
		...
		if (*(fs_info->macid_map + macid + MACID_MAX_8822E) == 0) {
			*(fs_info->macid_map + macid + MACID_MAX_8822E) = 1;
			(*macid_cnt)++;
		}
	}
		       ...
		       u16 **cur_fs, u8 *macid_cnt, u32 tx_agg_num)
{
	...
	status = chk_dma_mapping_8822e(adapter, cur_fs, qsel_first);
	if (status != HALMAC_RET_SUCCESS)
		return status;

	for (i = 0; i < tx_agg_num; i++) {
		/*QSEL parser*/
		status = chk_qsel_8822e(adapter, qsel_first, pkt, macid_cnt);
		if (status != HALMAC_RET_SUCCESS)
			return status;
...
u32
get_sdio_int_lat_8822e(struct halmac_adapter *adapter)
{
...
enum halmac_ret_status
get_sdio_clk_cnt_8822e(struct halmac_adapter *adapter, u32 *cnt)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	u8 value8;
...
enum halmac_ret_status
set_sdio_wt_en_8822e(struct halmac_adapter *adapter, u8 enable)
{
...
enum halmac_ret_status
set_sdio_clk_mon_8822e(struct halmac_adapter *adapter,
		       enum halmac_sdio_clk_monitor monitor)
{
	...
	set_sdio_wt_en_8822e(adapter, 0);
