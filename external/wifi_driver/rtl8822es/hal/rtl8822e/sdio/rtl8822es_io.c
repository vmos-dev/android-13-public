...
#include "rtl8822es.h"		/* rtl8822es_get_interrupt(), rtl8822es_clear_interrupt() and etc. */
 ...
 *	_FAIL(0)	Fail
 */
u32 rtl8822es_read_port(struct dvobj_priv *d, u32 cnt, u8 *mem)
{
...
static u32 sdio_read_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	return rtl8822es_read_port(pintfhdl->pintf_dev, cnt, recvbuf->pbuf);
}
 ...
 *	_FAIL(0)	Fail
 */
u32 rtl8822es_write_port(struct dvobj_priv *d, u32 cnt, u8 *mem)
{
	u32 txaddr, txsize;
	u32 ret = _FAIL;
...
static u32 sdio_write_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	ret = rtl8822es_write_port(d, cnt, xmitbuf->pdata);
...
static struct recv_buf *sd_recv_rxfifo(PADAPTER adapter, u32 size)
{
	...
	/* 2. alloc skb */
	pkt = rtl8822es_alloc_recvbuf_skb(recvbuf, bufsz);
	if (!pkt) {
	...
	ret = rtl8822es_read_port(d, bufsz, rbuf);
	if (_FAIL == ret) {
		RTW_ERR("%s: read port FAIL!\n", __FUNCTION__);
		rtl8822es_free_recvbuf_skb(recvbuf);
		rtw_enqueue_recvbuf(recvbuf, &recvpriv->free_recv_buf_queue);
		return NULL;
	}
...
void sd_int_dpc(PADAPTER adapter)
{
	...
	if (phal->sdio_hisr & BIT_SDIO_AVAL_8822E)
		_rtw_up_sema(&adapter->xmitpriv.xmit_sema);

#endif /* CONFIG_SDIO_TX_ENABLE_AVAL_INT */

	if (phal->sdio_hisr & BIT_SDIO_CPWM1_8822E) {
		...
		/* report.state = rtw_read8(adapter, REG_SDIO_HCPWM1_V2_8822E); */
	...
	if (phal->sdio_hisr & BIT_SDIO_TXERR_8822E) {
		u32 status;
		u32 addr;

		addr = REG_TXDMA_STATUS_8822E;
	...
	if (phal->sdio_hisr & BIT_SDIO_TXBCNOK_8822E)
		RTW_INFO("%s: SDIO_HISR_TXBCNOK\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_TXBCNERR_8822E)
		RTW_INFO("%s: SDIO_HISR_TXBCNERR\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_RXFOVW_8822E)
		RTW_INFO("%s: Rx Overflow\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_RXERR_8822E)
		RTW_INFO("%s: Rx Error\n", __FUNCTION__);
#ifdef CONFIG_SDIO_MULTI_FUNCTION_COEX
#if DBG_SDIO_MULTI_FUNCTION_COEX
	if (phal->sdio_hisr & BIT_BT_INT_8822E)
		RTW_INFO("%s: SDIO bus resume to available\n", __FUNCTION__);
#endif
#endif

	if (phal->sdio_hisr & BIT_RX_REQUEST_8822E) {
		...
		if (!ex_hal_sdio_multi_if_bus_available(adapter)
			&& !(phal->sdio_hisr & BIT_BT_INT_8822E)
		) {
			#if DBG_SDIO_MULTI_FUNCTION_COEX
		...
		phal->sdio_hisr ^= BIT_RX_REQUEST_8822E;
		...
		do {
			...
			if (precvbuf) {
				rtl8822es_rxhandler(adapter, precvbuf);
			...
			rx_len = 0;
			rtl8822es_get_interrupt(adapter, NULL, &rx_len);
		} while (1);
...
void sd_int_hdl(PADAPTER adapter)
{
	...
	rtl8822es_get_interrupt(adapter, &phal->sdio_hisr, &phal->SdioRxFIFOSize);
	if (phal->sdio_hisr & phal->sdio_himr) {
		phal->sdio_hisr &= phal->sdio_himr;
		sd_int_dpc(adapter);
		rtl8822es_clear_interrupt(adapter, phal->sdio_hisr);
	} else {
...
{
	rtl8822es_disable_interrupt_but_cpwm2(adapter);
	return _SUCCESS;
}
...
u8 RecvOnePkt(PADAPTER adapter)
{
	...
	rtl8822es_get_interrupt(adapter, &phal->sdio_hisr,
				&phal->SdioRxFIFOSize);
