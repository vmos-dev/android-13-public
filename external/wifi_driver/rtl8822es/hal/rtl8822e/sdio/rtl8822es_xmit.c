 ...
 *	For MI call.
 */
s32 rtl8822es_dequeue_writeport(PADAPTER adapter)
{
 ...
 */
s32 rtl8822es_xmit_buf_handler(PADAPTER adapter)
{
		...
		while (rtw_end_of_queue_search(sta_phead, sta_plist) == _FALSE) {
			...
			while (rtw_is_list_empty(frame_phead) == _FALSE) {
				...
				if ((NULL == pxmitbuf)
					...
					if (pxmitbuf) {
						if (pxmitbuf->len > 0 && pxmitbuf->priv_data) {
							...
							pframe->agg_num = k;
							pxmitbuf->agg_num = k;
							rtl8822e_update_txdesc(pframe, pframe->buf_addr);
				...
				if (ret == _FAIL) {
					...
					k++;
					if (k != 1)
						rtl8822e_update_txdesc(pxmitframe, pxmitframe->buf_addr);
...
thread_return rtl8822es_xmit_thread(thread_context context)
{
	s32 ret;
	PADAPTER adapter;
	u8 thread_name[20] = {0};
 ...
 *	_FAIL		fail
 */
s32 rtl8822es_mgnt_xmit(PADAPTER adapter, struct xmit_frame *pmgntframe)
{
	s32 ret = _SUCCESS;
	struct xmit_priv *pxmitpriv;
	...
	rtl8822e_update_txdesc(pmgntframe, pmgntframe->buf_addr);
 ...
 */
#ifdef CONFIG_RTW_MGMT_QUEUE 
s32 rtl8822es_hal_mgmt_xmit_enqueue(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
 ...
 *	_FALSE	fail
 */
s32 rtl8822es_hal_xmit_enqueue(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
 ...
 */
s32 rtl8822es_hal_xmit(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	_irqL irql;
	s32 ret;
	...
	_enter_critical_bh(&pxmitpriv->lock, &irql);
	ret = rtl8822es_hal_xmit_enqueue(adapter, pxmitframe);
	_exit_critical_bh(&pxmitpriv->lock, &irql);
	if (ret != _TRUE) {
 ...
 *
 */
s32 rtl8822es_init_xmit_priv(PADAPTER adapter)
{
...
#endif
	rtl8822e_init_xmit_priv(adapter);
	return _SUCCESS;
}

void rtl8822es_free_xmit_priv(PADAPTER adapter)
{
	struct xmit_priv *pxmitpriv;
	struct xmit_buf *pxmitbuf;
	_queue *pqueue;
